...
describe("Login Controller", () => {
    ...
    it("/POST auth/login - should login user1", async () => {
        ...
        const refreshTokenCountBefore = await RefreshToken.count({ where: { user: fixtures.user1 } });
        expect(refreshTokenCountBefore).toEqual(1);

        const accessTokenCountBefore = await AccessToken.count({ where: { user: fixtures.user1 } });
        ...
        const refreshTokenCountAfter = await RefreshToken.count({ where: { user: fixtures.user1 } });
        expect(refreshTokenCountAfter).toEqual(refreshTokenCountBefore + 1);

        const accessTokenCountAfter = await AccessToken.count({ where: { user: fixtures.user1 } });
